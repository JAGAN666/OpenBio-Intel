...
import { Activity, AlertTriangle, Database, Loader2, Search, Sparkles } from "lucide-react";
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <header className="mb-6">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-sky-700">
            ...
            medical-rag · market intelligence
          </div>
          ...
          >
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-300 bg-white py-3 pl-10 pr-4 text-sm text-slate-900 placeholder:text-slate-400 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-100 disabled:bg-slate-50"
              />
            </div>
            <button
              type="submit"
              disabled={loading || query.trim().length < 3}
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-sky-700 px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-sky-800 disabled:cursor-not-allowed disabled:bg-slate-300"
            >
              {loading ? (
                <>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            {EXAMPLES.map((ex) => (
              <button
                ...
                className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs text-slate-600 transition-colors hover:border-sky-300 hover:text-sky-700 disabled:opacity-50"
              >
                {ex}
              </button>
            ))}
        ...
        {/* --- error ----------------------------------------------------- */}
        {error && !loading && (
          <section className="mb-8 flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-6 py-5">
        ...
        {!loading && !error && !data && (
          <section className="rounded-xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center">
            <Database className="mx-auto h-6 w-6 text-slate-300" aria-hidden />
